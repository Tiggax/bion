...
use crate::{
    ...
    regressor::{Group, Mode, Param, RegressorNode, Target},
};
    ...
    fn left_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            sim_changed = sim_changed
                || ui
                    .collapsing("Simulation", |ui| self.sim.view(ui))
                    .body_returned
                    .unwrap_or(false);
            ...
            ui.horizontal(|ui| {
                if (ui.button("Load data")).clicked() {
                    if let Some(path) = rfd::FileDialog::new().pick_file() {
                        ...
                        let file = if let Ok(content) = fs::read_to_string(path) {
            ...
            ui.horizontal(|ui| {
                ...
                ui.selectable_value(
                    &mut self.minimization_param.target,
                    Target::FeedRate,
                    "Feed rate",
                );
                ui.selectable_value(
            ...
            ui.horizontal(|ui| {
                ...
                ui.selectable_value(
                    &mut self.minimization_param.mode,
                    Mode::Single(Group::VCD),
                    "VCD",
                );
                ui.selectable_value(
                    &mut self.minimization_param.mode,
                    Mode::Single(Group::Glucose),
                    "Glucose",
                );
                ui.selectable_value(
                    &mut self.minimization_param.mode,
                    Mode::Single(Group::Glutamin),
                    "Glutamin",
                );
                ui.selectable_value(
                    &mut self.minimization_param.mode,
                    Mode::Single(Group::DO),
                    "DO",
                );
                ui.selectable_value(
                    &mut self.minimization_param.mode,
                    Mode::Single(Group::Product),
                    "Product",
                );
                //}
            });
            ...
            if nodes_are_not_empty {
                ...
                for typ in [
                    ...
                    let target = match typ {
                        ...
                        Group::DO => self.sim_graphs.O2.clone(),
            ...
            if sim_changed || self.sim_graphs.vcd.is_empty() {
                ...
                let mut stepper =
                    ode_solvers::Rk4::new(self.sim.clone(), 0., initial_state, MINUTES, STEP);
    ...
    fn center_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        let plot_resp = my_plot.show(ui, |plot_ui| {
            ...
            if let Some(points) = do_points {
                plot_ui.points(points.radius(4.).color(Color32::LIGHT_BLUE));
            }

            // product
            let product_points = plot_points.pop();
            if let Some(points) = product_points {
                plot_ui.points(points.radius(4.).color(Color32::GOLD));
            }

            // Glutamin
            let glut_points = plot_points.pop();
            if let Some(points) = glut_points {
                plot_ui.points(points.radius(4.).color(Color32::YELLOW));
            }

            // glucose
            let gluc_points = plot_points.pop();
            if let Some(points) = gluc_points {
                plot_ui.points(points.radius(4.).color(Color32::GREEN));
            }

            // vcd
            let vcd_points = plot_points.pop();
            if let Some(points) = vcd_points {
                plot_ui.points(points.radius(4.).color(Color32::RED));
            }
            ...
            let DO: Vec<[f64; 2]> = self
                .sim_graphs
                .c_O2
                .clone()
                .into_iter()
                .map(|[x, y]| [x, (y / self.sim.oxigen_saturation()) * 100.])
                .collect();
