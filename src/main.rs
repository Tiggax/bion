#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")] // hide console window on Windows in release
#[allow(non_snake_case)]
pub mod base;
/// # Implementation of the model
/// the implementation of the model and all of the structs used to model parts of the simulation.
/// It also includes the view functions for the egui struct edit view.
pub mod model;
pub mod regressor;
pub mod ui;

use eframe::egui;
use ui::{app::BionApp, Front};

/// Main function that initializes logging and sets up the Eframe app
fn main() -> Result<(), eframe::Error> {
    env_logger::init(); // Log to stderr (if you run with `RUST_LOG=debug`).
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default().with_inner_size([1024.0, 768.0]),
        ..Default::default()
    };
    eframe::run_native(
        "My egui App",
        options,
        Box::new(|_cc| Box::<BionApp>::default()),
    )
}

/// # App eframe for Bion app
/// this implements the functions for viewing the app
impl eframe::App for BionApp {
    /// Update function that takes the frame, context and the BionApp ui and renders left panel and a central panel
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::SidePanel::left("options").show(ctx, |ui| self.left_panel(ui, ctx));
        egui::CentralPanel::default().show(ctx, |ui| self.center_panel(ui, ctx));
    }
}
